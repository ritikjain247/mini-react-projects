...
const USHeatmap = ({ onClick, onHover }: HeatmapProps) => {
  ...
  const clickHandler = (event: any) => {
    ...
    // onClick(event.target.getAttribute('name'));
    console.log(state);
  };

  const hoverHandler = (event: any) => {
    // remove all existing tooltips and styles first
    // onHover(event.target.getAttribute('name'));
    const state = event.target.getAttribute('name');
    console.log(state, event.pageX, event.pageY);
    document.getElementsByName(state).forEach((item) => item.setAttribute('fill', '#0000ff'));
  ...
  const hoverExitHandler = (event: any) => {
    // remove all existing tooltips and styles first
    // onHover(event.target.getAttribute('name'));
    const state = event.target.getAttribute('name');
    console.log(state);
    document.getElementsByName(state).forEach((item) => item.setAttribute('fill', '#D3D3D3'));
    // const tooltip = document.getElementById('tooltip');
    // if (tooltip) tooltip.style.display = 'none';
  };
      ...
      <svg className="us-state-map" xmlns="http://www.w3.org/2000/svg" width="959" height="593" viewBox="0 0 959 593">
        <g className="outlines">
          ...
          <g
            ...
            onMouseOut={hoverExitHandler}
          >
            <path className="DC1" fill="#D3D3D3" d="M801.8,253.8 l-1.1-1.6 -1-0.8 1.1-1.6 2.2,1.5z">
        ...
        />
        {/* <foreignObject width="400" height="150" x="10" y="10">
        <div
